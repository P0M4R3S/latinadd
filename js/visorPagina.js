...
function cargarPerfilUsuario(id) {
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.success) {
                renderizarPagina(response.pagina);
            } else {
                console.warn("No se pudo cargar la página: ", response.mensaje);
            }
        },
        error: function (xhr, status, error) {
            console.error("Error al conectar con el servidor para cargar la página");
            console.log("XHR:", xhr);
...
function renderizarPagina(pagina) {
    const img = pagina.imagen || "img/default.png";
    ...
    if (esPropia) {
        $("#btnAmigo").text("Eliminar página").off("click").on("click", function () {
            eliminarPagina();
        });
    } else {
        $("#btnAmigo").text(pagina.seguida ? "Dejar de seguir" : "Seguir +");
        $("#btnAmigo").off("click").on("click", function () {
            if (pagina.seguida) {
                dejarDeSeguirPagina();
            } else {
                seguirPagina();
...
function seguirPagina() {
    $.post('API/paginas/seguirPagina.php', {
        id: localStorage.getItem("idUsuario"),
        token: localStorage.getItem("tokenUsuario"),
        idpagina: idPagina
    }, function (response) {
        if (response.success) {
            $("#btnAmigo").text("Dejar de seguir");
        } else {
...
function dejarDeSeguirPagina() {
    $.post('API/paginas/dejarSeguirPagina.php', {
        id: localStorage.getItem("idUsuario"),
        token: localStorage.getItem("tokenUsuario"),
        idpagina: idPagina
    }, function (response) {
        if (response.success) {
            $("#btnAmigo").text("Seguir +");
        } else {
            console.warn(response.mensaje);
        }
    }, 'json');
...
function cargarPostsPagina(idpagina, indice) {
    cargando = true;
    $.ajax({
        url: 'API/paginas/cargarFeedPagina.php',
        type: 'POST',
        data: {
            id: localStorage.getItem("idUsuario"),
            token: localStorage.getItem("tokenUsuario"),
        ...
        error: function () {
            console.error("Error al cargar posts de la página");
            cargando = false;
        }
    });
}
...
$(document).on("click", ".btnComentar", function () {
    ...
    window.location.href = `visorPost.html?idpost=${idPost}&volver=pagina`;
});

$(document).on("click", ".btnCompartir", function () {
    numPost = $(this).data("id");
...
function eliminarPagina() {
    if (!confirm("¿Estás seguro de que deseas eliminar esta página? Esta acción no se puede deshacer.")) return;
    ...
    $.post('API/paginas/borrarPagina.php', {
        ...
        if (response.success) {
            ...
            console.warn("Error al eliminar la página:", response.mensaje);
            alert("No se pudo eliminar la página.");
        }
    }, 'json').fail(function () {
        console.error("Error al conectar con el servidor para eliminar la página");
    ...
    posts.forEach(post => {
        ...
        <div class="container-fluid post" data-id="${postId}">
            <div class="row perfilPost">
                ...
                <div class="col-8">
                    <span class="nombrePost" data-id="${post.usuario}" data-tipo="usuario">${nombre}</span><br>
